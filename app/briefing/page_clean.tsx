...
const highlights = [
  { emp: 'Aria', color: '#a78bfa', avatar: 'AR', win: 'Booked 28 discovery calls — 22% above target. Fintech segment responding best to the direct email sequence.' },
  { emp: 'Nova', color: '#34d399', avatar: 'NV', win: 'Published 31 pieces of content. Organic traffic up 24% MoM. The SEO keyword cluster is gaining traction.' },
  { emp: 'Opus', color: '#60a5fa', avatar: 'OP', win: 'Saved 47 hours of manual work. Onboarding SOP v2 is complete and ready for review.' },
];
...
const planned = [
  { emp: 'Aria', avatar: 'AR', color: '#a78bfa', task: 'Launch SMB outreach sequence — 50 prospects targeted' },
  { emp: 'Nova', avatar: 'NV', color: '#34d399', task: 'Publish September content calendar + 4 LinkedIn posts' },
  { emp: 'Opus', avatar: 'OP', color: '#60a5fa', task: 'Weekly performance report + inbox triage' },
];
    ...
    <div style={{ padding: '32px 36px', maxWidth: '800px', animation: 'fadeIn 0.3s ease' }}>
        ...
        <div style={{ padding: '8px 0' }}>
          {highlights.map((h, i) => (
            <div key={h.emp} style={{ display: 'flex', gap: '14px', padding: '16px 24px', borderBottom: i < highlights.length - 1 ? '1px solid var(--border)' : 'none', alignItems: 'flex-start' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '10px', overflow: 'hidden', flexShrink: 0, border: `1px solid ${h.color}20` }}><EmployeeAvatar id={h.emp.toLowerCase()} size={36} /></div>
              <div>
        ...
        <div style={{ padding: '8px 0' }}>
          {planned.map((p, i) => (
            <div key={p.emp} style={{ display: 'flex', gap: '14px', padding: '14px 24px', borderBottom: i < planned.length - 1 ? '1px solid var(--border)' : 'none', alignItems: 'center' }}>
              <div style={{ width: '28px', height: '28px', borderRadius: '8px', overflow: 'hidden', flexShrink: 0 }}><EmployeeAvatar id={p.emp.toLowerCase()} size={28} /></div>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}><span style={{ fontWeight: '700', color: 'var(--text-primary)' }}>{p.emp}</span> — {p.task}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ textAlign: 'center', padding: '20px 0' }}>
        <p style={{ fontSize: '12px', color: 'var(--text-dim)' }}>
          Sent every Monday at 8:00 AM · Surge AI Workforce Platform · <span style={{ color: 'var(--accent)', cursor: 'pointer' }}>View Dashboard →</span>
        </p>
      </div>
    </div>
  );
