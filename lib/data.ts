...
export async function getActivationStatus(): Promise<{ onboarded: boolean; activated: boolean; leadCount: number; draftCount: number }> {
  const companyId = await getMyCompanyId();
  if (!companyId) return { onboarded: false, activated: false, leadCount: 0, draftCount: 0 };
  const { data } = await supabase.from('companies').select('onboarding_complete, activated_at').eq('id', companyId).maybeSingle();
  // Live lead + draft counts so the dashboard can show the Research → Drafting transition during
  // the async activation run (drafts appear only after research, in the post-loop batch).
  const [{ count: leadCount }, { count: draftCount }] = await Promise.all([
    ...
    supabase.from('lead_drafts').select('id', { count: 'exact', head: true }).eq('company_id', companyId),
  ]);
  return { onboarded: data?.onboarding_complete === true, activated: data?.activated_at != null, leadCount: leadCount ?? 0, draftCount: draftCount ?? 0 };
}
