...
//   Pipeline  35% — qualified-or-better leads (ramps to 100 at 20), with ENGAGED
//                   leads (warm/booked — a real buying signal) weighted 2× so the
//                   number rewards pipeline that's actually responding, not just size
...
/**
 * @param {{ tasksCompleted?:number, tasksOpen?:number, leadsTotal?:number, leadsQualified?:number, leadsEngaged?:number, recentWork?:number }} input
 *        leadsEngaged = warm/booked leads (a real buying signal), weighted 2× in pipeline.
 * @returns {{ score:number|null, hasEnoughData:boolean, components:{key:string,label:string,value:number,weight:number}[] }}
 */
export function computeWorkforceScore({ tasksCompleted = 0, tasksOpen = 0, leadsTotal = 0, leadsQualified = 0, leadsEngaged = 0, recentWork = 0 } = {}) {
  ...
  if (leadsTotal > 0) {
    // Engaged (warm/booked) leads count double — pipeline that's responding is worth
    // more than pipeline that's just sitting there.
    components.push({ key: 'pipeline', label: 'Pipeline', value: clamp(leadsQualified * 5 + leadsEngaged * 10), weight: SCORE_WEIGHTS.pipeline });
  }
