...
function appBaseUrl() {
  return (process.env.NEXT_PUBLIC_APP_URL || 'https://getsurgehq.com').replace(/\/$/, '');
}
...
/**
 * Deliver one approved draft: load the draft + its lead, send via sendEmail (all the
 * gates apply), and on success mark the draft 'sent' and advance the lead to
 * 'contacted' with the next Lead-Lifeline follow-up in 3 days. Shared by the approve
 * route and the heartbeat warmup drip. Never throws.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
...
export async function deliverDraft(supabase, companyId, draftId) {
  ...
  const r = await sendEmail(supabase, { companyId, leadId: draft.lead_id, to, subject: draft.subject, text: draft.body });
  if (!r.ok) return r;
