...
// Live hands-free voice conversation (ElevenLabs Agents) is the premium COGS line —
// ElevenLabs ConvAI runs ~$0.08–0.10/min plus our LLM tokens. Metered per MINUTE of
// connected conversation; a failed/never-connected session charges ZERO (call site).
export const VOICE_HOURS_PER_MIN = 0.2;

/** Hour cost of `seconds` of live Atlas conversation (0 for none). 3-dp precision. */
export function voiceConvoHours(seconds) {
  const s = Math.max(0, Number(seconds) || 0);
  if (s === 0) return 0;
  return Math.round((s / 60) * VOICE_HOURS_PER_MIN * 1000) / 1000;
}
