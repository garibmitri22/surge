...
import { estimateHours, overtimeMessage, formatHours } from './pricing.mjs';

export { estimateHours, overtimeMessage };

/**
 * How to DISPLAY a company's hours, safely. The governing rule: a number is correct
 * or gone. Never render "{balance} of {allowance}" when balance > allowance (legacy
 * stacking / bad data) — clamp + relabel. Pure; shared by the widget + verify.
 * @param {{ balance?: number, allowance?: number, unlimited?: boolean }} summary
 * @returns {{ mode: 'unlimited'|'full'|'ratio', primary: string, secondary: string, pct: number }}
 */
export function hoursDisplay({ balance = 0, allowance = 0, unlimited = false } = {}) {
  if (unlimited) return { mode: 'unlimited', primary: 'Unlimited', secondary: '', pct: 100 };
  const b = Math.max(0, Number(balance) || 0);
  const a = Number(allowance) || 0;
  // balance >= allowance (or no allowance set) → show the balance alone as "available",
  // never the broken "198h of 70h" ratio.
  if (a <= 0 || b >= a) return { mode: 'full', primary: formatHours(b), secondary: 'team time available', pct: 100 };
  return { mode: 'ratio', primary: formatHours(b), secondary: `of ${formatHours(a)} team time left this month`, pct: Math.max(0, Math.min(100, (b / a) * 100)) };
}
