...
// Daily pipeline top-up tuning — SINGLE SOURCE OF TRUTH (import these; never re-hardcode).
// DAILY_TOPUP_TARGET: fresh leads per company per day. Sized to early email-warmup send
// capacity (~10/day) and the ~60s cron wall and a solo owner's review bandwidth; lift it
// as warmup raises the send ceiling. OPEN_LEAD_CEILING: the replenishing cap on UN-WORKED
// leads — stop topping up a pile that isn't being worked; it refills as the owner works it.
export const DAILY_TOPUP_TARGET = 10;
export const OPEN_LEAD_CEILING = 40;
