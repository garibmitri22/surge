...
import { agentId, agentIdFor, isAgentConfigured, voiceFor } from '../lib/voices.mjs';
...
// --- 3. Config gate -------------------------------------------------------------------------
console.log('\n--- 3. Centralized voice config (single source of truth) ---');
check('every agent has a config voice id (nothing hardcoded elsewhere)', ['atlas', 'aria', 'nova', 'opus'].every((e) => voiceFor(e).voiceId.length > 5));
check('agentIdFor() and back-compat agentId() agree on Atlas', agentIdFor('atlas') === agentId());
check('isAgentConfigured requires BOTH key and that agent id', typeof isAgentConfigured() === 'boolean');
if (!agentId()) check('no Atlas agent id set → agent not configured (expected pre-provision)', isAgentConfigured('atlas') === false);

// --- 4. Session route wiring (published agent, plain signed-URL connect) ---------------------
...
check('reads the live agent id from the centralized config', /from '@\/lib\/voices\.mjs'/.test(session) && /agentId\(\)/.test(session));
check('fetches an ElevenLabs signed URL for the configured agent', /get-signed-url\?agent_id=/.test(session));
check('returns the signed URL (plain connect — the published agent runs itself)', /ok: true, signedUrl/.test(session));
check('503 when the agent/keys are absent', /isAgentConfigured\(\)/.test(session) && /not_configured/.test(session) && /503/.test(session));

// --- 5. (Retained) custom-LLM bridge — present for an optional future full-brain mode --------
console.log('\n--- 5. /api/voice/llm bridge wiring (retained) ---');
...
check('connects with the signed URL (plain — the published agent runs itself)', /startSession\(\{ signedUrl/.test(ui) && !/customLlmExtraBody/.test(ui));
...
check('the orb reacts to LIVE conversation audio amplitude', /getOutputByteFrequencyData/.test(ui) && /PresenceOrb/.test(ui));

// Agent page surface: same live conversation + same /api/voice/session (one config everywhere).
const stage = read('../components/AgentStage.tsx');
check('agent page (AgentStage) also drives live conversation from the config', /useConversation\(/.test(stage) && /\/api\/voice\/session/.test(stage) && /getOutputByteFrequencyData/.test(stage));
...
console.log(failures === 0
  ? '\nVOICE-REALTIME VERIFICATION PASSED — centralized voice config, published-agent signed-URL connect, per-minute metering, and a hands-free client whose orb reacts to live audio. Live conversation = Mitri\'s mic test on prod.'
