...
export function HoursWidget() {
  ...
  const pct = h.allowance > 0 ? Math.max(0, Math.min(100, (h.balance / h.allowance) * 100)) : 0;
  const low = pct < 15;
  const color = low ? '#ef4444' : pct < 35 ? '#f59e0b' : '#22c55e';

  // Days left at the current pace (this month's burn so far).
  const day = new Date().getDate();
  const perDay = h.thisMonthUsed > 0 ? h.thisMonthUsed / day : 0;
  const daysLeft = perDay > 0 ? Math.floor(h.balance / perDay) : null;
