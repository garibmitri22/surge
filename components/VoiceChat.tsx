...
// JARVIS Phase 3 — hands-free live conversation with Atlas. Tap once; Atlas greets, you talk
// back continuously, interrupt him mid-sentence (native barge-in), and the live ElevenLabs
// agent runs Atlas's real brain via our custom-LLM webhook. Metered per minute on disconnect.
//
...
function VoiceChatInner({ employeeId = 'atlas' }: { employeeId?: string }) {
  ...
  async function start() {
    ...
    try {
      ...
      startSession({
        ...
        customLlmExtraBody: data.extraBody, // ElevenLabs forwards this to /api/voice/llm
      });
    } catch {
      setConnecting(false);
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '14px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '12px 16px', boxShadow: 'var(--shadow)' }}>
      <PresenceOrb employeeId={employeeId} state={orbState} size={56} level={isSpeaking ? 0.8 : live && !isMuted ? 0.25 : 0} aria-label="Atlas presence" />
