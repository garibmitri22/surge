...
- **Gap logged June 4 (validated by Mitri's own usage):** no brainstorm/ideas employee exists — Opus honestly flagged it when asked "who do I brainstorm with?" Decision: add a **Strategist / Chief-of-Staff** card to the pipeline (cheap, no build), measure demand via Month-3 waitlist clicks vs other pipeline employees. Do NOT build before revenue — 90-day rule. Until then, Cowork CEO is Mitri's brainstorm partner. Side note: Opus gave this honest answer with NO persona file yet (DB role only) — honesty guardrail holding; second feel-test data point passed.
...
## Build Order (Next Steps) — DECIDED June 3 (Mitri approved)
1. ✅ Wire up Supabase — replace mock data with real DB
2. 🔨 Auth — Supabase Auth login/signup + RLS lockdown (in progress)
3. **HQ Chat v1** — chat with employees, commands→tasks, real-data reports (see design above)
4. Onboarding interview — feeds memory layer (right behind HQ Chat; BOTH done before charging anyone)
5. Aria's first real action — live web research/lead qualification (HQ Chat v2)
6. Weekly CEO briefing email (Monday automation)
7. Billing — Stripe, $299/month per employee
